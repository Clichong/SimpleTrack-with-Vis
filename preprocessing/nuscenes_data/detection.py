...
parser.add_argument('--file_path', type=str, default='../../data/pred_data/swim_data/results_nusc.json')
...
def main(det_name, file_path, detection_folder, data_folder, mode):
    ...
    for scene_name in scene_names:
        if args.velo:
            with open(os.path.join(output_folder, '{:}.npz'.format(scene_name)), 'w') as f:
                json.dump(
                    {'bboxes':bboxes[scene_name], 'types':inst_types[scene_name], 'velos':velos[scene_name]}, f
                )
            ...
            with open(os.path.join(output_folder, '{:}.npz'.format(scene_name)), 'w') as f:
                json.dump(
                    {'bboxes':bboxes[scene_name], 'types':inst_types[scene_name]}, f
                )
            # np.savez_compressed(os.path.join(output_folder, '{:}.npz'.format(scene_name)),
