...
def associate_dets_to_tracks(dets, tracks, mode, asso, 
    ...
    unmatched_tracks = list()   # 未匹配的kalman pred
    ...
    for m in matched_indices:
        if dist_matrix[m[0], m[1]] > dist_threshold:    # giou不满足阈值则取消配对
            unmatched_dets.append(m[0])
            unmatched_tracks.append(m[1])
        else:
            matches.append(m.reshape(2))    # 按pair来存储
...
def bipartite_matcher(dets, tracks, asso, dist_threshold, trk_innovation_matrix):
    ...
    matched_indices = np.stack([row_ind, col_ind], axis=1)    # (tracks, dets)
